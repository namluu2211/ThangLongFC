...
describe('OfflinePlayerQueueService', () => {
  ...
  beforeEach(() => {
    // Clear persisted state BEFORE constructing service so restore() sees empty
    const ls = globalThis.localStorage as Storage | undefined;
    if(ls){ ls.clear(); }
    // Silence warn noise for dropped ops
  jest.spyOn(console, 'warn').mockImplementation(() => { /* noop for test */ return undefined; });
  ...
  test('flush processes operations with provided processor', async () => {
    ...
    // Service counts only successful operations; should equal number enqueued
    expect(result.processed).toBe(2);
    expect(result.remaining).toBe(0);
    expect(processed).toEqual(['create','delete']);
  });

  test('failed processor retries and eventually drops after max attempts (attempts counted separately)', async () => {
    service.enqueue({ type:'delete', id:'abc' });
    let attemptCount = 0;
    await service.flush(async () => { attemptCount++; throw new Error('fail'); });
    expect(service.length).toBe(0);
    expect(attemptCount).toBe(3); // equals configured maxAttempts
  });
});
